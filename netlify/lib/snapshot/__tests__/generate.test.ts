...
describe('generateSnapshot', () => {
  it('clusters across boards, reports losses, and records provenance', () => {
    ...
    expect(meta.parameters).toMatchObject({ anchor_count: 3, breadth_max: 1.5, h_breadth_days: 14, h_depth_days: 42, k: 5 })
  ...
  it('honours anchor_count with Math.min(N, size)', () => {
    const out = generateSnapshot(input({ anchorCount: 1 }))
    expect(out.clusters[0].anchor_node_ids).toEqual([`${B2}:7`])
    const big = generateSnapshot(input({ anchorCount: 10 }))
