...
/** Map node-band rows to the normalized admission shape (the node caller's
 * mapping in vadController) so the working-memory tests read like the real
 * call site. */
function nodeAdmissions(rows: RetrievalRow[], sourceBoard: string): WorkingMemoryAdmission[] {
  return rows.map((r) => ({
...
describe('buildRelatedMaterial', () => {
  ...
  it('renders curated saved nodes as bullets under one header (node-only corpus)', () => {
    const out = buildRelatedMaterial([
      row({ ref_id: 'n1', content: 'saved node one' }),
      row({ ref_id: 'n2', content: 'saved node two' }),
    ])!
    expect(out).toContain('Things you saved that relate to this edge:')
    expect(out).toContain('- saved node one')
    expect(out).toContain('- saved node two')
  })
...
describe('working memory (SURFACED THIS SESSION)', () => {
  ...
  it('admits each refId at most once across turns (novelty filter is the only dedupe)', () => {
    ...
    admitToWorkingMemory(memory, nodeAdmissions(turn1, 'b1'), 1)
    ...
    admitToWorkingMemory(memory, nodeAdmissions(turn2, 'b1'), 2)
  ...
  it('same refId from different boards coexist as distinct identities', () => {
    const memory = new Map<string, WorkingMemoryEntry>()
    admitToWorkingMemory(memory, nodeAdmissions([row({ ref_id: '16', content: 'board one node content' })], 'b1'), 1)
    admitToWorkingMemory(memory, nodeAdmissions([row({ ref_id: '16', content: 'board two node content' })], 'b2'), 2)
  ...
  it('same (refType, sourceBoard, refId) admitted twice is still once-per-identity', () => {
    ...
    admitToWorkingMemory(memory, nodeAdmissions([r], 'b1'), 1)
    admitToWorkingMemory(memory, nodeAdmissions([r], 'b1'), 2)
  ...
  it('overflow guard drops oldest entries (by surfacedAtTurn) and reports', () => {
    ...
    for (let turn = 1; turn <= 13; turn++) {
      admitToWorkingMemory(memory, nodeAdmissions([big(`r${turn}`)], 'b1'), turn, (info) => overflows.push(info))
    }
    expect(overflows).toHaveLength(0) // 13 × 600 = 7800 ≤ 8000

    admitToWorkingMemory(memory, nodeAdmissions([big('r14')], 'b1'), 14, (info) => overflows.push(info))
  ...
  it('onAdmit fires per entry with cumulative store stats', () => {
    ...
    admitToWorkingMemory(memory, nodeAdmissions(rows, 'b1'), 3, undefined, (info) => admits.push(info))
  ...
  it("renders 'node' and 'deposit' entries; throws on refTypes with no renderer yet", () => {
    ...
    // Deposits render as the same pre-formatted bullet (third-person prose,
    // no speaker prefix), interleaved with nodes under the one framing.
    const deposit: WorkingMemoryEntry = {
      refType: 'deposit',
      refId: 'd1',
      sourceBoard: 'unresolved',
      content: '- a prior-session deposit',
      surfacedAtTurn: 2,
    }
    expect(buildWorkingMemoryBlock([entry, deposit])).toBe(
      `${WORKING_MEMORY_FRAMING}\n\n- saved thing\n- a prior-session deposit`,
    )
  ...
  it('integration: node surfaced on the opening persists into an empty-retrieval follow-up prompt', () => {
    ...
    admitToWorkingMemory(memory, nodeAdmissions(novel, 'board-1'), 1)
